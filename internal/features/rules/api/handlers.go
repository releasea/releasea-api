	...
	if wantsPublish {
		if requiresApproval {
			approvalDoc, _, approvalErr := shared.CreateOrGetPendingGovernanceApproval(ctx, shared.GovernanceApprovalCreateParams{
				...
				Metadata: map[string]interface{}{
					...
					"action": map[string]interface{}{
						"kind":        operations.OperationTypeRulePublish,
						"ruleId":      ruleID,
						"serviceId":   serviceID,
						"environment": environment,
						"internal":    internal,
			...
			opDoc := bson.M{
				"_id":          operationID,
				"id":           operationID,
				"type":         operations.OperationTypeRulePublish,
				"resourceType": "rule",
				"resourceId":   ruleID,
				"status":       operations.StatusQueued,
				"createdAt":    now,
			...
			operationqueue.PublishOperationWithDispatchError(ctx, operationID)
		}
	}
...
func DeleteRule(c *gin.Context) {
	...
	if err := operationqueue.PublishOperation(ctx, opID); err != nil {
		operationqueue.RecordOperationDispatchError(ctx, opID, err)
