...
func cleanupManagedGithubRepos(ctx context.Context, projectID string, project bson.M, services []bson.M) error {
	...
	for key, repo := range repos {
		token := tokens[key]
		_, err := deleteManagedGithubRepo(ctx, token, repo, projectID)
		if err != nil {
			return err
		}
	}
...
func deleteManagedGithubRepo(ctx context.Context, token string, repo repoRef, projectID string) (bool, error) {
	...
	if marker.ProjectID != "" && marker.ProjectID != projectID {
		return false, nil
	}
