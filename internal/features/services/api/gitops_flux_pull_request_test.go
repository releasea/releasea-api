...
func TestCreateServiceFluxGitOpsPullRequestReturnsPullRequest(t *testing.T) {
	...
	previousOpen := openServiceFluxStarterPullRequest
	...
	openServiceFluxStarterPullRequest = func(_ context.Context, service bson.M, rules []bson.M, payload gitOpsFluxPullRequestPayload) (*scmmodels.DesiredStatePullRequestResponse, error) {
		...
		return &scmmodels.DesiredStatePullRequestResponse{
			...
			Number:     22,
			BaseBranch: "release",
			BranchName: "releasea/gitops/flux/checkout-api-20260328123500",
	...
	defer func() {
		...
		openServiceFluxStarterPullRequest = previousOpen
	}()

	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	request := httptest.NewRequest(http.MethodPost, "/services/svc-1/gitops/flux/pull-requests", strings.NewReader(`{"baseBranch":"release"}`))
	request.Header.Set("Content-Type", "application/json")
	ctx.Request = request
	ctx.Params = gin.Params{{Key: "id", Value: "svc-1"}}

	CreateServiceFluxGitOpsPullRequest(ctx)
	...
	if body.Number != 22 || len(body.FilePaths) != 4 {
