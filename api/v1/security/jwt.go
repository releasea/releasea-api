...
func AuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if authHeader == "" {
			shared.RespondError(c, http.StatusUnauthorized, "Missing authorization token")
			c.Abort()
			return
		}
		...
		if len(parts) != 2 || !strings.EqualFold(parts[0], "Bearer") {
			shared.RespondError(c, http.StatusUnauthorized, "Invalid authorization header")
			c.Abort()
			return
		}

		if isWorkerToken(parts[1]) {
			...
			registration, err := validateWorkerToken(parts[1])
			if err != nil {
		...
		if claims, err := parseWorkerAccessToken(parts[1]); err == nil && claims != nil {
