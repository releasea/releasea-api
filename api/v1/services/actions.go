...
func StartService(c *gin.Context) {
	...
	if isStaticSiteService(service) {
		count, err := handleStaticSiteStart(ctx, service, environment)
		if err != nil {
			log.Printf("[service.start] service=%s env=%s error=%v", serviceID, environment, err)
			shared.RespondError(c, http.StatusInternalServerError, "Failed to start static site")
			return
		}
		c.JSON(http.StatusAccepted, gin.H{"status": "queued", "rules": count})
		return
	}
	...
	activeOperation, err := shared.FindOne(ctx, shared.Collection(shared.OperationsCollection), activeFilter)
	if err == nil {
		c.JSON(http.StatusAccepted, gin.H{"operation": activeOperation})
		return
	}
	if !errors.Is(err, mongo.ErrNoDocuments) && err != nil {
		shared.RespondError(c, http.StatusInternalServerError, "Failed to check start queue")
		return
	}
	...
	if raw, ok := service["isActive"]; ok {
		prevIsActive = shared.BoolValue(raw)
	}
	...
	opDoc := bson.M{
		...
		"payload": bson.M{
			...
			"replicas":     replicas,
			"action":       "start",
			"prevStatus":   shared.StringValue(service["status"]),
	...
	if err := shared.InsertOne(ctx, shared.Collection(shared.OperationsCollection), opDoc); err != nil {
		shared.RespondError(c, http.StatusInternalServerError, "Failed to queue start")
		return
	}
...
func StopService(c *gin.Context) {
	...
	if isStaticSiteService(service) {
		count, err := handleStaticSiteStop(ctx, service, environment)
		if err != nil {
			log.Printf("[service.stop] service=%s env=%s error=%v", serviceID, environment, err)
			shared.RespondError(c, http.StatusInternalServerError, "Failed to stop static site")
			return
		}
		c.JSON(http.StatusAccepted, gin.H{"status": "queued", "rules": count})
		return
	...
	if !errors.Is(err, mongo.ErrNoDocuments) && err != nil {
		shared.RespondError(c, http.StatusInternalServerError, "Failed to check stop queue")
		return
	}
	...
	if err := shared.InsertOne(ctx, shared.Collection(shared.OperationsCollection), opDoc); err != nil {
		shared.RespondError(c, http.StatusInternalServerError, "Failed to queue stop")
		return
	}
...
func RestartService(c *gin.Context) {
	...
	if isStaticSiteService(service) {
		opDoc, deployDoc, err := queueStaticSiteDeploy(ctx, service, environment, shared.AuthDisplayName(c))
		if err != nil {
			log.Printf("[service.restart] service=%s env=%s error=%v", serviceID, environment, err)
			shared.RespondError(c, http.StatusInternalServerError, "Failed to redeploy static site")
			return
		}
		if deployDoc != nil {
			c.JSON(http.StatusAccepted, gin.H{"operation": opDoc, "deploy": deployDoc})
